...
kubectl patch deployment ${container_name} -p '{"spec":{"template":{"spec":{"containers":[{"name":"'${container_name}'","image": "registry.cn-hangzhou.aliyuncs.com/branch/gink:'${1}'","env":[{"name":"RESTART_TIME","value":"'$(date +%s)'"}]}]}}}}'
